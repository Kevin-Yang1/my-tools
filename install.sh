#!/usr/bin/env bash

set -euo pipefail

SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
SRC_DIR="${SCRIPT_DIR}/scripts"
TOOLS_SRC_DIR="${SRC_DIR}/tools"
BIN_DIR="${HOME}/.local/bin"

if [ ! -d "$SRC_DIR" ]; then
  echo "错误: 找不到脚本目录: $SRC_DIR" >&2
  exit 1
fi

command_name_for() {
  local base="$1"

  case "$base" in
    *.sh) base="${base%.sh}" ;;
    *.py) base="${base%.py}" ;;
    *.js) base="${base%.js}" ;;
    *.pl) base="${base%.pl}" ;;
    *.rb) base="${base%.rb}" ;;
  esac

  printf '%s\n' "${base//_/-}"
}

mkdir -p "$BIN_DIR"

install_wrapper() {
  local command_name="$1"
  local src="$2"
  local target="${BIN_DIR}/${command_name}"

  if [ -d "$target" ] && [ ! -L "$target" ]; then
    echo "错误: 目标路径是目录，无法安装: $target" >&2
    exit 1
  fi

  {
    printf '%s\n' '#!/usr/bin/env bash'
    printf '%s\n' '# generated by my-tools/install.sh'
    printf 'exec %q "$@"\n' "$src"
  } > "$target"

  chmod +x "$target"
  echo "已安装: $target -> $src"
}

install_aliases() {
  local command_name="$1"
  local src="$2"

  case "$command_name" in
    wait-and-run)
      install_wrapper "war" "$src"
      ;;
  esac
}

remove_obsolete_wrapper() {
  local command_name="$1"
  local old_src_fragment="$2"
  local target="${BIN_DIR}/${command_name}"

  [ -f "$target" ] || return
  grep -Fq '# generated by my-tools/install.sh' "$target" || return
  grep -Fq "$old_src_fragment" "$target" || return

  rm -f "$target"
  echo "已移除过期命令: $target"
}

installed_any=0
shopt -s nullglob
declare -A INSTALLED_COMMAND_SOURCES=()

install_from_dir() {
  local source_dir="$1"
  local src
  local command_name

  [ -d "$source_dir" ] || return

  for src in "$source_dir"/*; do
    [ -f "$src" ] || continue
    [ -x "$src" ] || continue

    command_name="$(command_name_for "$(basename "$src")")"
    if [ -n "${INSTALLED_COMMAND_SOURCES[$command_name]:-}" ] && [ "${INSTALLED_COMMAND_SOURCES[$command_name]}" != "$src" ]; then
      echo "错误: 命令名冲突: $command_name" >&2
      echo "  已存在来源: ${INSTALLED_COMMAND_SOURCES[$command_name]}" >&2
      echo "  新来源: $src" >&2
      exit 1
    fi

    INSTALLED_COMMAND_SOURCES[$command_name]="$src"
    install_wrapper "$command_name" "$src"
    install_aliases "$command_name" "$src"
    installed_any=1
  done
}

install_from_dir "$SRC_DIR"
install_from_dir "$TOOLS_SRC_DIR"
remove_obsolete_wrapper "exp-scheduler" "/scripts/exp_scheduler.py"

if [ "$installed_any" -eq 0 ]; then
  echo "错误: 没有发现可安装的可执行脚本" >&2
  exit 1
fi

case ":$PATH:" in
  *":$BIN_DIR:"*)
    ;;
  *)
    echo "提示: 你的 PATH 里还没有 $BIN_DIR"
    echo "可以加入：export PATH=\"\$HOME/.local/bin:\$PATH\""
    ;;
esac
