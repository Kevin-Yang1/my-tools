...
- 可选使用 `--backup-dir` 先备份被改动的 jsonl 和 sqlite 文件。

可执行示例:
- 先预览默认 `~/.codex` 下需要迁移的内容:
  `python3 scripts/tools/migrate_codex_provider_history.py`
- 真正执行迁移:
  `python3 scripts/tools/migrate_codex_provider_history.py --apply`
- 写入前先做备份:
  `python3 scripts/tools/migrate_codex_provider_history.py --backup-dir /tmp/codex-provider-backup --apply`
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action='append',
        default=None,
        help='保留不迁移的 provider key，可重复传入；默认仅保留目标 provider，openai 会被迁移',
    )
    parser.add_argument(
        '--state-db',
        type=Path,
...
def resolve_target_provider(cli_target_provider: str | None, config_status: ConfigStatus) -> str:
    if isinstance(cli_target_provider, str) and cli_target_provider.strip():
...
def main() -> int:
    ...
    target_provider = resolve_target_provider(args.target_provider, config_status)
    config_status.target_defined = target_provider in config_status.provider_keys
