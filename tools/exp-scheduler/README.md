...
- `gpu_idle_required_checks`：GPU 连续多少次满足可启动条件后才启动任务，默认 `6`。配合默认 `poll_interval_seconds = 5` 时，GPU 释放后约等待 30 秒稳定窗口。这个值也可以在网页“调控器”页实时修改并写入 SQLite 状态库。
- `auto_retry_max_retries`：自动重试次数。为 `0` 时不自动重试；大于 `0` 时，遇到 OOM / 资源类错误会重新排队重试。
- `auto_retry_delay_seconds`：自动重试前等待的秒数，只在 `auto_retry_max_retries > 0` 时生效。
...
如果调度服务停止，或任务进程被外部 `kill` 信号中断，调度器会把该任务放回原队列队首，下一次调度时自动重试。页面里的“取消任务”表示用户主动取消，不会自动重试。

运行中的任务日志会以只读终端实时显示。历史记录里的日志是任务结束后保存的纯文本日志；双击历史记录可以打开日志，再次双击关闭。

GPU 页面可以控制哪些 GPU 参与调度，也可以给单张 GPU 设置定时开启或关闭。“调控器”页面可以实时调整检测间隔和连续满足次数。监控页面会在内嵌终端中运行 `nvitop`。
