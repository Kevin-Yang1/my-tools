...
`exp-scheduler` 是一个面向单用户实验服务器的 GPU 任务调度器。你把命令加入队列后，它会在 GPU 连续多次满足可启动条件时自动启动任务，并通过一个只监听 `127.0.0.1` 的网页控制台提供增删改查、拖拽排序、取消任务、日志查看、环境模板复用、从现有 `conda` / `venv` 自动导入模板、给单个任务指定 GPU、实时调整全局可调度 GPU 白名单、调控器策略、紧急任务队列与手动抢占，以及按全局策略处理的 OOM / CUDA 资源类错误自动重试。其中运行中任务日志会以只读 PTY 终端展示，历史任务则继续使用纯文本日志视图。
...
- 调控器：实时编辑检测间隔和连续满足次数，降低 GPU 刚释放时的撞车概率
...
默认策略是 `poll_interval_seconds = 5` 且 `gpu_idle_required_checks = 6`，也就是 GPU 释放后需要连续约 30 秒满足可启动条件，调度器才会启动下一个任务。
...
- 网页“调控器”页可以实时修改检测间隔和连续满足次数，设置会写入 SQLite 状态库并立即生效
...
- 历史里展示的是最后一次尝试结果
- 修改 `config.toml` 后需要重启 `exp-scheduler serve` 或 `systemd --user` 服务；网页“调控器”里的检测间隔和连续次数会实时生效，不需要重启
