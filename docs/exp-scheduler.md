...
`exp-scheduler` 是一个面向单用户实验服务器的 GPU 任务调度器。你把命令加入队列后，它会在 GPU 连续多次满足可启动条件时自动启动任务，并通过一个只监听 `127.0.0.1` 的网页控制台提供增删改查、拖拽排序、取消任务、日志查看、环境模板复用、从现有 `conda` / `venv` 自动导入模板、给单个任务指定 GPU、实时调整全局可调度 GPU 白名单、关闭 GPU 后按连续空闲时长自动恢复可用、外部 kill 后 GPU 冷却、调控器策略、紧急任务队列与手动抢占，以及按全局策略处理的 OOM / CUDA 资源类错误自动重试。其中运行中任务日志会以只读 PTY 终端展示，历史任务则继续使用纯文本日志视图。
...
- 调控器：实时编辑检测间隔、连续满足次数、外部 kill 后 GPU 冷却和自动重试策略，降低 GPU 刚释放时的撞车概率
...
- 多次运行/自动重试的任务会在日志面板里按 attempt 切换查看；运行中任务默认显示实时终端，也可以切回之前的 attempt 日志
- 可以删除单个 attempt 的日志文件，删除前网页会要求确认；删除只影响该日志文件，不会删除任务记录，也不会允许删除正在运行且仍在写入的当前日志
...
- 如果任务被外部信号中断，调度器会把任务放回队首，并让对应 GPU 进入 `external_kill_gpu_cooldown_seconds` 冷却时间；冷却期间这张 GPU 不再接收新任务
- 网页“调控器”页可以实时修改检测间隔、连续满足次数、空闲自动恢复、外部 kill 后 GPU 冷却和自动重试策略，设置会写入 SQLite 状态库并立即生效
